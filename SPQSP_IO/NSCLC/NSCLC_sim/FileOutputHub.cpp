...
{
	_log.close();
	_generalStats_core.close();
	_generalStats_margin.close();
	_odeStatsT.close();
...
void FileOutputHub::setup(long seed, string baseDirectory){
	...
	statsFileName = _outDirBase + "/" + StatsFilePrefix + "core_" + to_string(_seed) + ".csv";
	_generalStats_core.open(statsFileName, ios::out | ios::app);
	statsFileName = _outDirBase + "/" + StatsFilePrefix + "margin_" + to_string(_seed) + ".csv";
	_generalStats_margin.open(statsFileName, ios::out | ios::app);
...
ofstream& FileOutputHub::getStatsFstream(int i){ 
	if (i == 0)
	{
		return _generalStats_core; 
	}
	else{
		return _generalStats_margin; 
	}
}
