...
##########################
#render
bu.render_config(x=720, y=720, sample=16, use_gpu = False)
...
studio.set_subject_size(4)
studio.adjust_light('key', strength = 2e2, rot_x=np.pi/3)
studio.adjust_light('fill', strength = 1e2, rot_x=np.pi/3)
studio.adjust_light('rim', strength = 2e2, rot_x=np.pi/3)
...
for frame_nr in frames:
    ...
    arrow.reset_location()
    sce.render.filepath = working_dir +'/scratch/surf/' + str(frame_nr)
